...
export class ProjectService {
  ...
  protected convertDateFromClient(project: IProject): IProject {
    const copy: IProject = Object.assign({}, project, {
      expiredDate: project.expiredDate != null && project.expiredDate.isValid() ? project.expiredDate.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((project: IProject) => {
        project.expiredDate = project.expiredDate != null ? moment(project.expiredDate) : null;
      });
    }
    return res;
  }
